...
- Embedded the Voiceflow chat widget snippet globally (see `index.html` in this bundle).

## Replace These Placeholders Before Launch
1. Voiceflow widget project ID: update `YOUR_VOICEFLOW_PROJECT_ID` in `/updated-site/index.html`.
2. Formspree endpoint: swap `FORM_ID` for your live ID inside the quote form in the compiled bundle (`assets/index-*.js`) and in the source file `src/components/Contact.tsx` if you plan further edits.
3. Social URLs and OG image: adjust the placeholder links/paths in `index.html` and JSON-LD to match your live profiles and sharing image.
